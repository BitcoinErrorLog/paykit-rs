...
pub async fn run(storage_dir: &Path, verbose: bool) -> Result<()> {
    ...
    ui::info(&format!("Found {} identity(ies) to migrate", old_identities.len()));
    ...
    for name in &old_identities {
        ...
        match old_manager.load(name) {
            Ok(identity) => {
                match secure_manager.save(&identity, name).await {
                    Ok(()) => {
                        migrated += 1;
                        if verbose {
    ...
    ui::separator();
    ui::success(&format!("Migrated {}/{} identities", migrated, old_identities.len()));
