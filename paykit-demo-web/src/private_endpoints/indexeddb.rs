...
impl PrivateEndpointStore for WasmPrivateEndpointStorage {
    async fn save(&self, endpoint: PrivateEndpoint) -> StorageResult<()> {
        ...
        let promise: js_sys::Promise = js_value.dyn_into().map_err(|_| {
            StorageError::Other("Failed to convert transaction to Promise".to_string())
        })?;
    ...
    async fn list_for_peer(&self, peer: &PublicKey) -> StorageResult<Vec<PrivateEndpoint>> {
        ...
        for i in 0..js_array.length() {
            ...
            let stored: StoredEndpoint = serde_wasm_bindgen::from_value(item).map_err(|e| {
                StorageError::Serialization(format!("Deserialization failed: {}", e))
            })?;
