...
fn bench_x25519_key_agreement(c: &mut Criterion) {
    ...
    // Pre-generate secrets for the benchmark loop
    let secrets: Vec<_> = (0..1000)
        .map(|_| EphemeralSecret::random_from_rng(rand::thread_rng()))
        .collect();
    let mut idx = 0;
    
    c.bench_function("x25519_key_agreement", |b| {
        b.iter(|| {
            // Use pre-generated secrets since EphemeralSecret is consumed
            let secret = &secrets[idx % secrets.len()];
            idx += 1;
            let shared = secret.diffie_hellman(black_box(&bob_public));
            black_box(shared)
        })
    });
}
...
fn bench_chacha20_poly1305_encrypt(c: &mut Criterion) {
    ...
    group.bench_function("64_bytes", |b| {
        b.iter(|| {
            let ciphertext = cipher.encrypt(nonce, black_box(small_message.as_slice())).unwrap();
    ...
    group.bench_function("1kb", |b| {
        b.iter(|| {
            let ciphertext = cipher.encrypt(nonce, black_box(medium_message.as_slice())).unwrap();
            black_box(ciphertext)
        })
    });
    ...
    group.bench_function("16kb", |b| {
        b.iter(|| {
            let ciphertext = cipher.encrypt(nonce, black_box(large_message.as_slice())).unwrap();
            black_box(ciphertext)
        })
    });
...
fn bench_chacha20_poly1305_decrypt(c: &mut Criterion) {
    ...
    c.bench_function("chacha20_poly1305_decrypt_1kb", |b| {
        b.iter(|| {
            let plaintext = cipher.decrypt(nonce, black_box(ciphertext.as_slice())).unwrap();
            black_box(plaintext)
        })
    });
}
