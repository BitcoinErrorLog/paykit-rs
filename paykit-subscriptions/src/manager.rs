...
use crate::{
    ...
    NonceStore, PaymentRequest, PaymentRequestResponse, RequestFilter, RequestStatus, Result,
...
use paykit_lib::{transport::UnauthenticatedTransportRead, PublicKey};
use std::sync::Arc;

/// Messages for subscription protocol
///
/// Note: Size difference between variants is acceptable here as these are
/// protocol message types that are rarely stored in large collections.
/// Boxing variants would complicate serialization and is not justified
/// for the typical usage pattern (single message processing).
...
impl SubscriptionManager {
    ...
    /// Poll for new payment requests from a peer.
    ///
    /// This queries the Pubky directory at `/pub/paykit.app/v0/requests/{peer_id}/`
    /// to discover all pending payment requests, fetch their content, and return
    /// new requests that haven't been processed yet.
    ///
    /// # Arguments
    ///
    /// * `peer` - The public key of the peer to poll for requests
    /// * `transport` - An unauthenticated transport for fetching public data
    ///
    /// # Returns
    ///
    /// A vector of `PaymentRequest` objects that are new (not in local storage).
    ///
    /// # Example
    ///
    /// ```rust,no_run
    /// # use paykit_subscriptions::{SubscriptionManager, Result};
    /// # use paykit_lib::{PublicKey, transport::UnauthenticatedTransportRead};
    /// # async fn example(manager: &SubscriptionManager, peer: &PublicKey, transport: &impl UnauthenticatedTransportRead) -> Result<()> {
    /// let new_requests = manager.poll_requests(peer, transport).await?;
    ...
    pub async fn poll_requests(
        &self,
        peer: &PublicKey,
        transport: &dyn UnauthenticatedTransportRead,
    ) -> Result<Vec<PaymentRequest>> {
        // Build the path to the peer's requests directory
        let path = format!("/pub/paykit.app/v0/requests/{}/", peer);

        // List all request files in the directory
        let entries = transport.list_directory(peer, &path).await?;
        ...
        for entry in entries {
            ...
            // Fetch the request file
            let file_path = format!("{}{}", path, entry);
            if let Some(data) = transport.fetch_file(peer, &file_path).await? {
                // Parse the request
                match serde_json::from_slice::<PaymentRequest>(&data) {
                    Ok(request) => {
                        ...
                        let existing = self.storage.list_requests(filter).await?;
                        let is_new = !existing.iter().any(|r| r.request_id == request.request_id);
                    ...
                    Err(err) => {
                        // Log parsing error (can be replaced with tracing in production)
                        eprintln!("Failed to parse request {}: {}", entry, err);
                        continue;
                    }
                }
            }
