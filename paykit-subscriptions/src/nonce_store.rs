...
impl NonceStore {
    ...
    pub fn count(&self) -> usize {
        // SAFETY: Mutex poisoning indicates a thread panic while holding the lock,
        // which represents an unrecoverable program state. The NonceStore's integrity
        // cannot be guaranteed after poisoning, so propagating the panic is correct.
        // This prevents silently continuing with potentially corrupted nonce tracking,
        // which could allow replay attacks. Poisoning is extremely rare in practice
        // and indicates a serious bug that should be fixed rather than masked.
        let nonces = self
            .used_nonces
            .read()
